...
@Component
public class AddNewProfessorCallback implements Callback {

  private final ChatStateManager chatStateManager;

  @Autowired
  public AddNewProfessorCallback(ChatStateManager chatStateManager) {
  ...
  public @NonNull SendMessage execute(Update update, Map<String, String> callbackData) {
    ...
    chatStateManager.setChatState(chatId, ChatState.AWAITING_PROFESSOR_NAME);
    return SendMessage.builder().chatId(chatId).text("Введите полное имя преподавателя").build();
  }

  @Override
  public Action getAccordingAction() {
    return Action.ADD_NEW_PROFESSOR;
  }
}
