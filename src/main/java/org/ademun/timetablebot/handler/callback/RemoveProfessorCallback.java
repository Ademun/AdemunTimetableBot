...
@Component
public class RemoveProfessorCallback implements Callback {

  private final GroupService groupService;

  @Autowired
  public RemoveProfessorCallback(GroupService groupService) {
  ...
  public @NonNull SendMessage execute(Update update, Map<String, String> callbackData) {
    ...
    groupService.removeProfessor(group.getGroupId(), professorId);
    return SendMessage.builder().chatId(chatId)
        .text("Преподаватель удален из списка вашей группы").build();
  }

  @Override
  public Action getAccordingAction() {
    return Action.REMOVE_PROFESSOR;
  }
}
