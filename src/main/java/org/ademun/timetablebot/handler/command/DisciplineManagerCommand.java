...
@Component
public class DisciplineManagerCommand implements Command {
  ...
  private InlineKeyboardMarkup getInlineKeyboardMarkup() {
    ...
    rows.add(new InlineKeyboardRow(
        InlineKeyboardButton.builder().text("Добавить")
            .callbackData("action=" + Action.MANAGER_ADD_DISCIPLINE.name())
            .build()));
    rows.add(new InlineKeyboardRow(
        InlineKeyboardButton.builder().text("Удалить")
            .callbackData("action=" + Action.MANAGER_REMOVE_DISCIPLINE.name())
            .build()));
